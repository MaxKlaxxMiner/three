...
func NewWebGLState(gl GLContext, extensions *WebGLExtensions) *WebGLState {
	r := &WebGLState{gl: gl, extensions: extensions, enabledCapabilities: map[int]bool{}}

	glVersion := gl.getParameterStr(gl.Const("VERSION"))
...
func (s *WebGLState) Enable(id int) {
...
func (s *WebGLState) Disable(id int) {
	if !s.enabledCapabilities[id] {
		return
	}
	if id == 0 {
...
func (s *WebGLState) SetFlipSided(flipSided bool) {
	if s.currentFlipSided == flipSided {
		return
	}
	if flipSided {
		s.gl.Call("frontFace", s.gl.Const("CW"))
	} else {
		s.gl.Call("frontFace", s.gl.Const("CCW"))
	}
	s.currentFlipSided = flipSided
}

func (s *WebGLState) SetCullFace(cullFace consts.CullFace) {
	if s.currentCullFace == cullFace {
		return
	}

	if cullFace != consts.CullFaceNone {
		s.Enable(s.gl.Const("CULL_FACE"))

		if cullFace == consts.CullFaceBack {
			s.gl.Call("cullFace", s.gl.Const("BACK"))
		} else if cullFace == consts.CullFaceFront {
			s.gl.Call("cullFace", s.gl.Const("FRONT"))
		} else {
			s.gl.Call("cullFace", s.gl.Const("FRONT_AND_BACK"))
		}
	} else {
		s.Disable(s.gl.Const("CULL_FACE"))
	}
