	...
	buildPlane := func(u, v, w int, udir, vdir, width, height, depth float64, gridX, gridY, materialIndex int) {
		...
		for iy := 0; iy < gridY1; iy++ {
			...
			for ix := 0; ix < gridX1; ix++ {
				...
				vector.SetComponent(w, util.If(depth > 0, 1.0, -1.0))
		...
		//			scope.addGroup( groupStart, groupCount, materialIndex ); todo
...
func (b *BoxGeometry) Copy(a *BoxGeometry) *BoxGeometry {
	//		super.copy( source ); todo
